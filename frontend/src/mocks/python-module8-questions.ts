...
const QUESTION_SEEDS: QuestionSeed[] = [
  {
    ...
    productionExample:
      'The general "two pointers converging from both ends" pattern (here, and in Module 3\'s Three Sum) is the same technique used in production for problems like "find the pair of timestamps in a sorted event log with the maximum gap under a constraint" — recognizing WHEN an array is sorted-or-sortable and a pairwise relationship is being optimized is the transferable interview skill.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ndef length_of_longest_substring(s: str) -> int:\n    last_seen = {}   # char -> most recent index seen at\n    left = 0\n    best = 0\n    for right, ch in enumerate(s):\n        if ch in last_seen and last_seen[ch] >= left:\n            left = last_seen[ch] + 1   # jump left past the previous occurrence\n        last_seen[ch] = right\n        best = max(best, right - left + 1)\n    return best\n```\nDry run length_of_longest_substring("abcabcbb"): right=0 \'a\' new, window[0,0] len1 best1; right=1 \'b\' new, window[0,1] len2 best2; right=2 \'c\' new, window[0,2] len3 best3; right=3 \'a\' seen at 0>=left(0) -> left=1, window[1,3] len3 best3; right=4 \'b\' seen at1>=left(1) -> left=2, window[2,4] len3 best3; right=5 \'c\' seen at2>=left(2) -> left=3, window[3,5] len3 best3; right=6 \'b\' seen at4, 4<left(3)? No 4>=3 -> left=5, window[5,6] len2; right=7 \'b\' seen at6>=left(5) -> left=7, window[7,7] len1. Final best=3 ("abc").\n\nThe critical correctness detail: checking `last_seen[ch] >= left` (not just `ch in last_seen`) is what prevents the window from incorrectly shrinking based on a STALE occurrence that already fell OUTSIDE the current window — a very common off-by-one/logic bug in this exact problem.',
    productionExample:
  ...
  },
  {
    ...
    productionExample:
      'A monotonic stack is the standard technique behind "stock span" problems (how many consecutive prior days had a price <= today\'s) and browser-history-style "next larger event" analytics — the daily_temperatures pattern generalizes directly to "how long until metric X exceeds its current value again" dashboards.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nclass ListNode:\n    def __init__(self, value=0, next=None):\n        self.value = value\n        self.next = next\n\ndef reverse_list(head: ListNode | None) -> ListNode | None:\n    prev = None\n    current = head\n    while current:\n        next_node = current.next   # save before overwriting\n        current.next = prev         # reverse the link\n        prev = current\n        current = next_node\n    return prev   # prev is the new head\n\ndef find_middle(head: ListNode | None) -> ListNode | None:\n    slow = fast = head\n    while fast and fast.next:\n        slow = slow.next          # moves 1 step\n        fast = fast.next.next      # moves 2 steps\n    return slow   # when fast reaches the end, slow is at the middle\n\ndef has_cycle(head: ListNode | None) -> bool:\n    slow = fast = head\n    while fast and fast.next:\n        slow = slow.next\n        fast = fast.next.next\n        if slow is fast:            # they meet -> there IS a cycle\n            return True\n    return False   # fast reached the end -> no cycle\n\ndef merge_two_sorted(a: ListNode | None, b: ListNode | None) -> ListNode | None:\n    dummy = ListNode()\n    tail = dummy\n    while a and b:\n        if a.value <= b.value:\n            tail.next, a = a, a.next\n        else:\n            tail.next, b = b, b.next\n        tail = tail.next\n    tail.next = a or b   # attach whichever list still has remaining nodes\n    return dummy.next\n```\nWhy the slow/fast pointer meets INSIDE a cycle (Floyd\'s algorithm intuition): once both pointers are inside the cycle, the fast pointer gains exactly 1 relative step on the slow pointer every iteration (moving 2 vs 1) — since the cycle is finite, the gap between them (mod cycle length) must eventually reach 0, meaning they occupy the SAME node, guaranteeing a meeting within at most one full lap of the cycle.\n\nThe `dummy` node pattern in `merge_two_sorted` avoids special-casing "what is the head of the merged list" — you always start with a throwaway placeholder node and return `dummy.next` at the end, which is a very common, worth-memorizing linked-list idiom that eliminates a whole class of edge-case branching.',
    productionExample:
      'Cycle detection (Floyd\'s algorithm) is not just a toy problem — it is the real technique used to detect infinite loops in any pointer-chain/reference-graph structure with O(1) extra memory, such as detecting a cyclic dependency in a manually-linked data structure or a circular reference chain, without needing to allocate a full "visited" hash set.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ndef binary_search(nums: list[int], target: int) -> int:\n    left, right = 0, len(nums) - 1\n    while left <= right:\n        mid = (left + right) // 2\n        if nums[mid] == target:\n            return mid\n        elif nums[mid] < target:\n            left = mid + 1\n        else:\n            right = mid - 1\n    return -1\n\ndef first_occurrence(nums: list[int], target: int) -> int:\n    left, right = 0, len(nums) - 1\n    result = -1\n    while left <= right:\n        mid = (left + right) // 2\n        if nums[mid] == target:\n            result = mid\n            right = mid - 1   # keep searching LEFT for an earlier occurrence\n        elif nums[mid] < target:\n            left = mid + 1\n        else:\n            right = mid - 1\n    return result\n\ndef search_rotated(nums: list[int], target: int) -> int:\n    left, right = 0, len(nums) - 1\n    while left <= right:\n        mid = (left + right) // 2\n        if nums[mid] == target:\n            return mid\n        if nums[left] <= nums[mid]:            # LEFT half is properly sorted\n            if nums[left] <= target < nums[mid]:\n                right = mid - 1\n            else:\n                left = mid + 1\n        else:                                    # RIGHT half is properly sorted instead\n            if nums[mid] < target <= nums[right]:\n                left = mid + 1\n            else:\n                right = mid - 1\n    return -1\n```\nWhy `(left + right) // 2` can be a subtle bug in OTHER languages (not Python): in fixed-width-integer languages, `left + right` can overflow for very large indices — the safer form is `left + (right - left) // 2`. Python\'s arbitrary-precision `int` makes this a non-issue, but it is worth mentioning as language-comparison awareness in a senior interview.\n\n`search_rotated`\'s key insight: even though the WHOLE array is not sorted, at least ONE of the two halves around any midpoint always IS properly sorted — checking `nums[left] <= nums[mid]` tells you which half is sorted, and then a normal range check on that sorted half tells you whether to search there or in the other (unsorted-looking but still binary-searchable) half.',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nclass TreeNode:\n    def __init__(self, val=0, left=None, right=None):\n        self.val, self.left, self.right = val, left, right\n\ndef preorder(root):    # root, then left, then right\n    if root is None:\n        return []\n    return [root.val] + preorder(root.left) + preorder(root.right)\n\ndef inorder(root):      # left, then root, then right — SORTED order for a valid BST\n    if root is None:\n        return []\n    return inorder(root.left) + [root.val] + inorder(root.right)\n\ndef postorder(root):    # left, then right, then root — useful for safe deletion/cleanup order\n    if root is None:\n        return []\n    return postorder(root.left) + postorder(root.right) + [root.val]\n\nfrom collections import deque\n\ndef level_order(root):\n    if root is None:\n        return []\n    result = []\n    queue = deque([root])\n    while queue:\n        level_size = len(queue)\n        level_values = []\n        for _ in range(level_size):     # process exactly ONE full level at a time\n            node = queue.popleft()\n            level_values.append(node.val)\n            if node.left:\n                queue.append(node.left)\n            if node.right:\n                queue.append(node.right)\n        result.append(level_values)\n    return result\n\ndef height(root):\n    if root is None:\n        return 0\n    return 1 + max(height(root.left), height(root.right))\n\ndef is_balanced(root) -> bool:\n    def check(node):   # returns -1 as a sentinel for "already unbalanced", else the height\n        if node is None:\n            return 0\n        left_height = check(node.left)\n        if left_height == -1:\n            return -1\n        right_height = check(node.right)\n        if right_height == -1:\n            return -1\n        if abs(left_height - right_height) > 1:\n            return -1\n        return 1 + max(left_height, right_height)\n    return check(root) != -1\n```\nWhy `is_balanced` uses a `-1` sentinel instead of a separate helper checking balance at every node: a naive approach recomputes `height()` for every node from scratch at every level, giving O(n²) in the worst case (a skewed tree); folding the balance CHECK into the same recursive pass that computes height achieves O(n) — computing each subtree\'s height exactly once and propagating "already found unbalanced" up through the recursion via the sentinel, short-circuiting further work.\n\nThe `level_size = len(queue)` snapshot at the start of each `while` iteration in `level_order` is the key trick for grouping nodes by LEVEL — without it, you would get a flat single list of all values in BFS order, with no way to tell where one level ends and the next begins.',
    productionExample:
      'Level-order (BFS) traversal is the standard technique for "render an org chart / file-system tree level by level" UI features, and for shortest-path-in-unweighted-graph problems (Module 8\'s graph section) generally — the "process one full level, then the next" structure is directly reused there.',
    bestPractices: [
