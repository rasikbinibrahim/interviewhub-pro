...
const QUESTION_SEEDS: QuestionSeed[] = [
  {
    ...
    deepExplanation:
      'Execution pipeline in CPython:\n\n```text\nsource.py\n  -> tokenizer -> AST\n  -> compiled to bytecode (.pyc, cached under __pycache__/)\n  -> executed by the CPython bytecode interpreter (a stack machine)\n```\n\nYou can inspect the bytecode directly:\n\n```python\nimport dis\n\ndef add(a, b):\n    return a + b\n\ndis.dis(add)\n# LOAD_FAST a\n# LOAD_FAST b\n# BINARY_ADD\n# RETURN_VALUE\n```\n\nOther implementations solve different problems: PyPy trades startup time and memory for JIT speed on hot loops; Jython/IronPython target the JVM/.NET to interoperate with those ecosystems; MicroPython targets microcontrollers. For a backend interview, the practical takeaway is: CPython bytecode execution + the GIL (see the concurrency module) explain most of Python\'s performance characteristics — why pure-Python loops are slow relative to C-backed operations (numpy, list comprehensions using C-implemented iteration), and why native extensions (written in C/Rust) are used for hot paths.',
    productionExample:
      'Production teams pick CPython by default (widest library/ecosystem compatibility — most C extensions like numpy/psycopg2 only support CPython\'s C-API). PyPy is chosen selectively for long-running, CPU-bound, pure-Python services (e.g. a custom parser or simulation) where the JIT warm-up cost is amortized and C-extension dependencies are minimal or PyPy-compatible.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ntype(True)          # <class \'bool\'>\nisinstance(True, int)  # True — bool IS-A int\nTrue + True          # 2\n\n0.1 + 0.2 == 0.3      # False — binary float representation error\nfrom decimal import Decimal\nDecimal("0.1") + Decimal("0.2") == Decimal("0.3")  # True\n\nb = b"hello"          # bytes — immutable\nba = bytearray(b"hello")  # bytearray — mutable\nba[0] = 72             # ok\n```\n\nInterview-relevant nuance: Python `int` has no fixed-width overflow (unlike Java/C), so `2**1000` just works, at the cost of extra bookkeeping/memory. `bool` being an `int` subclass explains why `sum([True, True, False])` is `2`, and why `True == 1` and `hash(True) == hash(1)` (meaning `True` and `1` collide as dict keys).',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ndef count_vowels(s: str) -> int:\n    vowels = set("aeiouAEIOU")\n    return sum(1 for ch in s if ch in vowels)\n\ndef remove_duplicate_chars(s: str) -> str:\n    seen = set()\n    result = []\n    for ch in s:\n        if ch not in seen:\n            seen.add(ch)\n            result.append(ch)\n    return "".join(result)\n\nfrom collections import Counter\n\ndef first_non_repeating(s: str) -> str | None:\n    counts = Counter(s)\n    for ch in s:\n        if counts[ch] == 1:\n            return ch\n    return None\n```\n\nDry run first_non_repeating("swiss"): counts = {s:3, w:1, i:1}. Scan: \'s\' count 3 skip, \'w\' count 1 -> return \'w\'.\n\nremove_duplicate_chars preserves first-seen order using a `set` for O(1) membership checks plus a list buffer (never string `+=` in a loop — quadratic).',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      'Problem: ["flower", "flow", "flight"] -> "fl". ["dog", "racecar", "car"] -> "" (no common prefix).\n\nApproach — vertical scanning:\n```python\ndef longest_common_prefix(strs: list[str]) -> str:\n    if not strs:\n        return ""\n    for i, ch in enumerate(strs[0]):\n        for other in strs[1:]:\n            if i >= len(other) or other[i] != ch:\n                return strs[0][:i]\n    return strs[0]\n```\n\nDry run ["flower", "flow", "flight"]: i=0 \'f\' matches all; i=1 \'l\' matches all; i=2 \'o\' vs "flow"[2]=\'o\' ok, vs "flight"[2]=\'i\' MISMATCH -> return strs[0][:2] = "fl".\n\nAlternative Pythonic approach uses `os.path.commonprefix(strs)` in production code, or `min`/`max` of the list (since Python compares strings lexicographically, the common prefix of the whole list is bounded by the common prefix of just the min and max strings) for an O(S) trick without nested loops.',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ndef compress(s: str) -> str:\n    if not s:\n        return s\n    parts = []\n    count = 1\n    for i in range(1, len(s) + 1):\n        if i < len(s) and s[i] == s[i - 1]:\n            count += 1\n        else:\n            parts.append(s[i - 1] + str(count))\n            count = 1\n    compressed = "".join(parts)\n    return compressed if len(compressed) < len(s) else s\n\ndef expand(s: str) -> str:\n    import re\n    result = []\n    for ch, count in re.findall(r"([a-zA-Z])(\\d+)", s):\n        result.append(ch * int(count))\n    return "".join(result)\n```\n\nDry run compress("aaabbc"): i=1 \'a\'==\'a\' count=2; i=2 \'a\'==\'a\' count=3; i=3 \'b\'!=\'a\' push "a3" count=1; i=4 \'b\'==\'b\' count=2; i=5 \'c\'!=\'b\' push "b2" count=1; i=6 (end) push "c1" -> "a3b2c1" (6 chars, same length as input "aaabbc" (6 chars) — per the "only compress if shorter" rule, in this case they tie, so implementation detail: use `<` not `<=`, meaning here it would return the original "aaabbc").',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ndef word_count(s: str) -> int:\n    return len(s.split())\n\ndef word_frequency(s: str):\n    from collections import Counter\n    return Counter(s.lower().split())\n\ndef capitalize_words(s: str) -> str:\n    return " ".join(word[:1].upper() + word[1:] for word in s.split())\n```\n\nWhy not just `s.title()`? `"don\'t stop".title()` gives `"Don\'T Stop"` — it capitalizes after every non-alpha character including the apostrophe, which is usually wrong. The manual `word[:1].upper() + word[1:]` approach only touches the first character of each whitespace-delimited token, avoiding that bug.\n\nDry run capitalize_words("the sky is blue"): split -> ["the","sky","is","blue"]; each -> "The","Sky","Is","Blue"; join -> "The Sky Is Blue".',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ns = "python"\ns[0]        # \'p\'\ns[-1]       # \'n\'\ns[1:4]      # \'yth\'\ns[::-1]     # \'nohtyp\' (reverse)\ns[::2]      # \'pto\' (every 2nd char)\n\nname = "Ada"\nage = 30\nprint(f"{name} is {age} years old")            # "Ada is 30 years old"\nprint(f"{age:03d}")                              # "030" — format spec\nprint(f"{3.14159:.2f}")                          # "3.14"\nprint(f"{name!r}")                               # "\'Ada\'" — repr conversion\n```\n\nSlicing never raises IndexError even for out-of-range bounds (`s[100:200]` on a 6-char string just returns `\'\'`), unlike single-index access (`s[100]` raises IndexError) — a frequently-tested asymmetry. f-strings are preferred over `%`-formatting and `str.format()` for new code: they are faster (compiled to bytecode that evaluates expressions directly) and more readable since the expression sits inline instead of needing positional/keyword substitution.',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ns = "café"\nb_utf8 = s.encode("utf-8")     # b\'caf\\xc3\\xa9\' — \'é\' takes 2 bytes in UTF-8\nlen(s)                          # 4 (code points)\nlen(b_utf8)                     # 5 (bytes)\n\nb_utf8.decode("utf-8")          # "café" — round-trips correctly\ns.encode("ascii")               # UnicodeEncodeError: \'ascii\' codec can\'t encode \'é\'\ns.encode("ascii", errors="ignore")   # b\'caf\' — silently drops non-ASCII (usually wrong for production)\ns.encode("ascii", errors="replace")  # b\'caf?\'\n```\n\nThe classic interview trap: `len(str)` counts code points, not bytes, and not always "visual characters" either (combining characters/emoji with modifiers can be multiple code points that render as one glyph). When reading/writing files or sockets, always be explicit about encoding (`open(path, encoding="utf-8")`) rather than relying on the platform default, which differs between Linux/macOS (usually UTF-8) and some Windows locales.',
    productionExample:
  ...
  },
  {
    ...
    productionExample:
      'CLI tools built with `input()`/`print()` are rare in production services (which use structured logging and typed config instead), but the type-conversion discipline shown here — never trust incoming data\'s type, always convert/validate explicitly — is exactly the same discipline needed when parsing untyped request bodies (e.g. query string params, which also always arrive as strings) in a web backend.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      'Type system: Python (dynamic, strong) rejects `"1" + 1`; JavaScript (dynamic, weak) coerces it to `"11"`; Java (static, strong) rejects it at COMPILE time, before the program even runs — the earliest possible error surfacing of the three.\n\nConcurrency: Python\'s GIL means CPU-bound multi-threading does not parallelize (Module 7 covers this in depth) — CPU-bound work needs multiprocessing or native extensions; I/O-bound work is fine with threads or asyncio. JavaScript has no GIL because it is single-threaded by design (one call stack, event loop, non-blocking I/O via callbacks/promises) — "parallelism" there means Web Workers/separate processes, not shared-memory threads. Java threads are real OS threads with true parallel execution, at the cost of much more complex correctness concerns (locks, races, memory visibility) than Python or single-threaded JS ever have to reason about.\n\nPerformance: Java\'s JIT (HotSpot) generally makes long-running CPU-bound Java code fastest of the three; Node\'s V8 JIT makes JS surprisingly fast for a dynamic language; CPython has no JIT by default, so raw interpreter loops are the slowest — but in practice, most backend services are I/O-bound (waiting on DB/network), where this difference matters far less than architecture (Module 7\'s async concepts) and where Python\'s ecosystem/productivity often wins.\n\nMemory: Python uses reference counting (immediate deallocation when refcount hits 0) plus a generational cycle collector for reference cycles; Java uses a tracing GC (no refcounting, periodic mark-and-sweep/generational collection, with GC pauses to reason about); JS engines use similar tracing GC strategies to Java.',
    productionExample:
