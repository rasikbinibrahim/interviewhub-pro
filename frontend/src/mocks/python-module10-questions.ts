...
const QUESTION_SEEDS: QuestionSeed[] = [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nfrom enum import Enum\nfrom fastapi import FastAPI\nfrom pydantic import BaseModel, Field\n\napp = FastAPI()\n\nclass Role(str, Enum):\n    admin = "admin"\n    member = "member"\n\nclass Address(BaseModel):\n    street: str\n    city: str\n    zip_code: str = Field(..., pattern=r"^\\d{5}$")\n\nclass UserCreate(BaseModel):\n    email: str\n    name: str\n    role: Role = Role.member                    # optional, has a default\n    address: Address                             # nested model, required\n    tags: list[str] = []                         # optional list, defaults empty\n    metadata: dict[str, str] = {}                 # optional dict\n\n@app.post("/users")\nasync def create_user(payload: UserCreate):\n    return {"id": 1, **payload.model_dump()}\n```\n\nRequest/response examples:\n\n```text\nPOST /users\nBody: {\n  "email": "ada@example.com",\n  "name": "Ada Lovelace",\n  "address": {"street": "1 Analytical Engine Way", "city": "London", "zip_code": "90210"},\n  "tags": ["vip"]\n}\n-> 201-worthy body accepted, role defaults to "member", metadata defaults to {}\n-> 200 {"id": 1, "email": "ada@example.com", "name": "Ada Lovelace", "role": "member",\n        "address": {"street": "...", "city": "London", "zip_code": "90210"}, "tags": ["vip"], "metadata": {}}\n\nPOST /users\nBody: {"email": "ada@example.com", "name": "Ada", "address": {"street": "x", "city": "y", "zip_code": "abc"}}\n-> 422 {"detail": [{"loc": ["body", "address", "zip_code"], "msg": "String should match pattern \'^\\\\d{5}$\'", ...}]}\n```\n\nThe error `loc` array (`["body", "address", "zip_code"]`) demonstrates recursive validation: FastAPI/Pydantic walks into the nested `Address` model and reports the exact FIELD PATH where validation failed, not just "the body is invalid" — this is what makes Pydantic validation errors genuinely actionable for API consumers building form-level error display, rather than a single opaque failure message.',
    productionExample:
      'A user-registration endpoint with a nested `address` object, an `role` enum with a safe default, and optional `tags`/`metadata` is a realistic production shape — the nested-path validation errors let a frontend map `body.address.zip_code` directly onto the corresponding form field\'s error state without any custom error-parsing logic on the client.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nfrom fastapi import FastAPI\nfrom pydantic import BaseModel, EmailStr\n\napp = FastAPI()\n\nclass UserInDB(BaseModel):          # the FULL internal representation, as loaded from the database\n    id: int\n    email: EmailStr\n    name: str\n    hashed_password: str            # must NEVER be exposed via the API\n\nclass UserResponse(BaseModel):      # the PUBLIC-safe response schema — deliberately excludes hashed_password\n    id: int\n    email: EmailStr\n    name: str\n\n@app.get("/users/{user_id}", response_model=UserResponse)\nasync def get_user(user_id: int) -> UserInDB:\n    # even though this returns the FULL UserInDB object (with hashed_password!),\n    # FastAPI filters the response down to only UserResponse\'s fields before serializing\n    return UserInDB(id=user_id, email="ada@example.com", name="Ada", hashed_password="$2b$12$...")\n```\n\nAPI example — the actual bytes on the wire NEVER contain `hashed_password`, regardless of what the function returned:\n\n```text\nGET /users/42\n-> 200 {"id": 42, "email": "ada@example.com", "name": "Ada"}\n```\n\nThis is why separating request/response/internal schemas (`UserCreate` for input, `UserResponse` for output, `UserInDB`/an ORM model for internal storage) is the standard FastAPI pattern rather than reusing ONE model everywhere — a single shared model either forces the client to send fields it should never set (like `id` or `hashed_password` on create) or risks the reverse mistake of accidentally returning fields it should never see, entirely dependent on programmer discipline instead of the framework enforcing it structurally.\n\n`response_model` also supports `response_model_exclude_unset=True` (only include fields the caller actually set, useful for PATCH-style partial responses), `response_model_exclude={"field"}` for one-off exclusions, and works transparently with lists (`response_model=list[UserResponse]`) and nested models.',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nfrom fastapi import APIRouter, Header, HTTPException, Query, status\nfrom pydantic import BaseModel, Field\n\nrouter = APIRouter(prefix="/api/v1/products", tags=["Products"])\n\nclass ProductUpdate(BaseModel):\n    name: str | None = None\n    price: float | None = Field(None, gt=0)\n    category: str | None = None\n\nclass Product(BaseModel):\n    id: int\n    name: str\n    price: float\n    category: str\n\n_db: dict[int, Product] = {\n    1: Product(id=1, name="Laptop", price=999.0, category="Electronics"),\n}\n\ndef _require_bearer(authorization: str | None) -> str:\n    if authorization is None or not authorization.startswith("Bearer "):\n        raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail="Missing bearer token")\n    return authorization.removeprefix("Bearer ")\n\n@router.patch("/{product_id}", response_model=Product)\nasync def update_product(\n    product_id: int,\n    payload: ProductUpdate,\n    authorization: str | None = Header(None),\n    notify: bool = Query(False, description="Send a notification about this change"),\n):\n    _require_bearer(authorization)\n\n    product = _db.get(product_id)\n    if product is None:\n        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Product not found")\n\n    update_data = payload.model_dump(exclude_unset=True)   # only fields the client actually sent\n    updated = product.model_copy(update=update_data)\n    _db[product_id] = updated\n\n    if notify:\n        pass   # would enqueue a notification — see Module 14\'s BackgroundTasks question\n\n    return updated\n```\n\nAPI example:\n\n```text\nPATCH /api/v1/products/1?notify=true\nHeaders: Authorization: Bearer abc123\nBody: {"price": 1099.0}\n-> 200 {"id": 1, "name": "Laptop", "price": 1099.0, "category": "Electronics"}\n\nPATCH /api/v1/products/1\n(no Authorization header)\n-> 401 {"detail": "Missing bearer token"}\n\nPATCH /api/v1/products/999\nHeaders: Authorization: Bearer abc123\nBody: {"price": 50.0}\n-> 404 {"detail": "Product not found"}\n```\n\nThe key correctness detail is `payload.model_dump(exclude_unset=True)`: without `exclude_unset=True`, every field NOT included in the client\'s request body would be present in `update_data` as its default (`None`), and merging that into the existing product would incorrectly overwrite `name`/`category` with `None` even though the client only meant to update `price` — `exclude_unset=True` is exactly what makes this a true, safe PARTIAL update (PATCH semantics) rather than an accidental full overwrite.',
    productionExample:
  ...
  },
  {
    ...
    productionExample:
      'A document-management product\'s "drag and drop 10 files" upload feature uses exactly this partial-success pattern — users routinely mix valid and invalid files in one drag-drop batch, and a hard all-or-nothing failure on the whole request would be a frustrating UX forcing them to identify and remove the bad file manually before retrying everything.',
    bestPractices: [
  ...
  },
  {
    ...
    productionExample:
      'A billing dashboard\'s "export all transactions as CSV" feature for an account with hundreds of thousands of rows uses exactly this batched-generator-plus-StreamingResponse pattern — generating the entire CSV in memory first (a naive `"\\n".join(all_rows)` approach) would both delay the time-to-first-byte until the whole export finished and risk out-of-memory errors on large accounts, whereas streaming starts delivering data to the browser almost immediately and holds only one batch in memory at a time.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nimport base64\nimport json\nfrom fastapi import APIRouter, HTTPException, Query, status\nfrom pydantic import BaseModel\n\nrouter = APIRouter(prefix="/api/v1/orders", tags=["Orders"])\n\nclass Order(BaseModel):\n    id: int\n    status: str\n    created_at: str   # ISO-8601, sortable as a string\n    total: float\n\n# assume sorted ascending by (created_at, id) — the stable sort order the cursor relies on\n_db: list[Order] = [\n    Order(id=1, status="paid", created_at="2024-01-01T00:00:00", total=50.0),\n    Order(id=2, status="pending", created_at="2024-01-02T00:00:00", total=75.0),\n    Order(id=3, status="paid", created_at="2024-01-03T00:00:00", total=20.0),\n]\n\ndef _encode_cursor(order: Order) -> str:\n    payload = json.dumps({"created_at": order.created_at, "id": order.id})\n    return base64.urlsafe_b64encode(payload.encode()).decode()\n\ndef _decode_cursor(cursor: str) -> tuple[str, int]:\n    try:\n        payload = json.loads(base64.urlsafe_b64decode(cursor.encode()))\n        return payload["created_at"], payload["id"]\n    except Exception:\n        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="Invalid cursor")\n\n@router.get("/search")\nasync def search_orders(\n    status_filter: str | None = Query(None, alias="status"),\n    after: str | None = Query(None, description="Opaque cursor from a previous page\'s next_cursor"),\n    limit: int = Query(20, ge=1, le=100),\n):\n    items = _db\n    if status_filter:\n        items = [o for o in items if o.status == status_filter]   # filter BEFORE paginating\n    if after:\n        after_created_at, after_id = _decode_cursor(after)\n        items = [o for o in items if (o.created_at, o.id) > (after_created_at, after_id)]\n\n    page = items[:limit]\n    next_cursor = _encode_cursor(page[-1]) if len(page) == limit else None\n    return {"items": page, "next_cursor": next_cursor, "count": len(page)}\n```\n\nAPI example:\n\n```text\nGET /api/v1/orders/search?status=paid&limit=1\n-> 200 {"items": [{"id":1,"status":"paid","created_at":"2024-01-01T00:00:00","total":50.0}],\n        "next_cursor": "eyJjcmVhdGVkX2F0IjogIjIwMjQtMDEtMDFUMDA6MDA6MDAiLCAiaWQiOiAxfQ==", "count": 1}\n\nGET /api/v1/orders/search?status=paid&after=eyJjcmVhdGVkX2F0IjogIjIwMjQtMDEtMDFUMDA6MDA6MDAiLCAiaWQiOiAxfQ==&limit=1\n-> 200 {"items": [{"id":3,"status":"paid","created_at":"2024-01-03T00:00:00","total":20.0}],\n        "next_cursor": null, "count": 1}\n\nGET /api/v1/orders/search?after=not-valid-base64\n-> 400 {"detail": "Invalid cursor"}\n```\n\nThe key correctness detail: FILTERING happens BEFORE cursor comparison, and the cursor comparison operates on the SAME sort key (`created_at`, `id`) that defines the collection\'s stable order — reversing that order (comparing the cursor before filtering) or using a sort key inconsistent with the actual query order would silently produce wrong/incomplete pages. `next_cursor` is `None` exactly when the page came back SHORTER than `limit`, which is the correct, simple signal that there is no more data (no separate "has_more" flag or COUNT query needed).',
    productionExample:
