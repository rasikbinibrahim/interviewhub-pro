...
const QUESTION_SEEDS: QuestionSeed[] = [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\ndef add_item(item, bucket=[]):\n    bucket.append(item)\n    return bucket\n\nprint(add_item("a"))   # [\\\n\nStep 1 — Understand the topic.\nTopic: The mutable default argument trap\n\nStep 2 — Easy method.\nIdentify the core Python rule or algorithm pattern, trace a small example, and only then optimize.\n\nStep 3 — WITHOUT BUILT-IN / CORE Python:\n```python\ndef add_item_bad(item: str, bucket: list[str] = []) -> list[str]:\n    bucket.append(item)\n    return bucket\n\nprint(add_item_bad("a"))\nprint(add_item_bad("b"))\n```\n\nStep 4 — WITH BUILT-IN / PYTHONIC / STANDARD-LIBRARY Python:\n```python\ndef add_item(item: str, bucket: list[str] | None = None):\n    if bucket is None:\n        bucket = []\n    bucket.append(item)\n    return bucket\n\nprint(add_item("a"))\nprint(add_item("b"))\n```\n\nStep 5 — Example result:\n```text\n[a]\n[a, b] in the bad example; [a] then [b] in the fixed example\n```\n\nStep 6 — Complexity / trade-off:\nThe fix creates a fresh list per call.\n\nStep 7 — Edge cases:\nCheck empty input, None, invalid types, boundaries, duplicates, mutation/aliasing, exceptions, large input, and concurrency/lifecycle behavior where applicable.\n\nStep 8 — Interview takeaway:\nExplain the rule first, demonstrate the core implementation, compare the Pythonic/standard-library option, then give complexity, failure modes, and production trade-offs.',
    productionExample:
      'This exact bug has shipped to production as "a cache/log/accumulator that inexplicably grows across unrelated requests" — e.g. a `def handle_request(logs=[])` helper silently accumulating log entries from EVERY request into one shared list because no caller ever passes `logs` explicitly, leaking data between unrelated requests and growing unbounded memory over the life of the process.\n\nCoding practice: first explain the core/manual approach for **The mutable default argument trap**, then compare it with the Pythonic or standard-library form where appropriate.',
    bestPractices: [
