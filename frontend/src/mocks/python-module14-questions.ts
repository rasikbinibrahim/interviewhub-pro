...
const QUESTION_SEEDS: QuestionSeed[] = [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nimport asyncio\nimport csv\nimport io\nfrom fastapi import FastAPI\nfrom fastapi.responses import StreamingResponse\n\napp = FastAPI()\n\nasync def generate_large_csv():\n    header = io.StringIO()\n    csv.writer(header).writerow(["id", "name", "email"])\n    yield header.getvalue()\n    for i in range(1, 100_001):                 # simulate 100k rows without holding them all in memory\n        row = io.StringIO()\n        csv.writer(row).writerow([i, f"user{i}", f"user{i}@example.com"])\n        yield row.getvalue()\n        if i % 10_000 == 0:\n            await asyncio.sleep(0)               # yield control back to the event loop periodically\n\n@app.get("/export/users.csv")\nasync def export_users():\n    return StreamingResponse(\n        generate_large_csv(),\n        media_type="text/csv",\n        headers={"Content-Disposition": "attachment; filename=users.csv"},\n    )\n\nasync def event_stream():\n    counter = 0\n    while True:\n        counter += 1\n        yield f"data: {{\\"tick\\": {counter}}}\\n\\n"   # SSE framing: \\"data: <payload>\\\\n\\\\n\\"\n        await asyncio.sleep(1)\n\n@app.get("/events")\nasync def sse_endpoint():\n    return StreamingResponse(event_stream(), media_type="text/event-stream")\n```\n\nWhy streaming matters for memory: a naive `return {"rows": [build_all_100k_rows()]}` endpoint must construct the ENTIRE response body in process memory before FastAPI can send even the first byte, which for a genuinely large export can spike memory usage dramatically and delays time-to-first-byte until the whole thing is ready. `StreamingResponse` instead sends each chunk as soon as the generator yields it, using HTTP\'s chunked transfer encoding — memory usage stays roughly constant (proportional to one chunk) regardless of total export size, and the client starts receiving data immediately.\n\nSSE vs WebSockets: SSE is one-directional (server -> client only), works over plain HTTP (no protocol upgrade, plays nicely with existing HTTP infrastructure/proxies, and auto-reconnects natively in browser `EventSource`), and is the simpler, lighter-weight choice whenever the client only needs to RECEIVE live updates (a live dashboard, notification feed) rather than also send messages — reach for a full WebSocket only when genuine bidirectional, low-latency communication is required (chat, collaborative editing).',
    productionExample:
      'A production analytics dashboard uses SSE for "live metric updates" (server pushes new numbers as they compute, client never sends anything back) precisely because it avoids WebSocket connection-management complexity while still getting real-time push, whereas the export-to-CSV feature on the same product uses `StreamingResponse` specifically to avoid buffering potentially hundreds of megabytes of report data in a single worker\'s memory.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nfrom datetime import datetime, timezone\nfrom fastapi import APIRouter, FastAPI, Request\nimport logging\n\nlogger = logging.getLogger("api.deprecation")\nv1_router = APIRouter(prefix="/api/v1", tags=["v1 (deprecated)"], deprecated=True)   # marks EVERY v1 route as deprecated in the OpenAPI docs\nv2_router = APIRouter(prefix="/api/v2", tags=["v2"])\n\nSUNSET_DATE = "Wed, 01 Apr 2026 00:00:00 GMT"\n\n@v1_router.get("/users/{user_id}")\nasync def get_user_v1(user_id: int, request: Request):\n    logger.warning("deprecated v1 endpoint called", extra={"path": request.url.path, "ua": request.headers.get("user-agent")})\n    return {"id": user_id, "name": "Ada"}   # old, less complete shape\n\n@v2_router.get("/users/{user_id}")\nasync def get_user_v2(user_id: int):\n    return {"id": user_id, "name": "Ada", "email": "ada@example.com", "created_at": "2024-01-01T00:00:00Z"}   # richer v2 shape\n\napp = FastAPI()\n\n@app.middleware("http")\nasync def add_deprecation_headers(request: Request, call_next):\n    response = await call_next(request)\n    if request.url.path.startswith("/api/v1"):\n        response.headers["Deprecation"] = "true"\n        response.headers["Sunset"] = SUNSET_DATE\n        response.headers["Link"] = \'</api/v2/users>; rel="successor-version"\'\n    return response\n\napp.include_router(v1_router)\napp.include_router(v2_router)\n```\n\nA real deprecation lifecycle has distinct phases: (1) ship v2 alongside v1, both fully functional; (2) mark v1 `deprecated=True` (shows a strikethrough/warning in Swagger UI) and start emitting `Deprecation`/`Sunset` headers plus structured logs/metrics on every v1 call, tagged with caller identity (API key, user-agent) where possible; (3) proactively notify remaining known v1 callers using the usage data from phase 2; (4) once metrics show v1 traffic has dropped to zero (or an acceptable, explicitly-accepted-risk residual), remove the v1 router entirely.\n\nThe `Sunset` HTTP header (RFC 8594) and `Deprecation` header are STANDARDIZED, machine-readable ways to signal an impending removal — well-behaved API clients/SDKs can programmatically detect and log/alert on these rather than a human having to notice a changelog entry.',
    productionExample:
  ...
  },
  {
    ...
    productionExample:
      'A mid-size SaaS platform typically runs 3–10 FastAPI pod replicas behind a managed load balancer (e.g. a cloud provider\'s ALB or an Nginx Ingress on Kubernetes), a managed Redis instance for caching and rate-limiting, a managed PostgreSQL primary with read replicas for reporting queries, and a small fleet of Celery workers consuming from a Redis or SQS-backed queue for email/webhook/report-generation workloads — each piece independently scalable based on its own bottleneck (API instance count for request throughput, worker count for background job backlog, DB replicas for read-heavy reporting load).',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nimport logging\nfrom fastapi import FastAPI, HTTPException, Request, status\nfrom fastapi.exceptions import RequestValidationError\nfrom fastapi.responses import JSONResponse\nfrom pydantic import BaseModel\n\nlogger = logging.getLogger("api")\napp = FastAPI()\n\nclass ErrorDetail(BaseModel):\n    code: str\n    message: str\n\nclass ErrorResponse(BaseModel):\n    error: ErrorDetail\n\ndef envelope(code: str, message: str) -> dict:\n    return ErrorResponse(error=ErrorDetail(code=code, message=message)).model_dump()\n\n@app.exception_handler(HTTPException)\nasync def http_exception_handler(request: Request, exc: HTTPException):\n    return JSONResponse(status_code=exc.status_code, content=envelope(f"HTTP_{exc.status_code}", str(exc.detail)))\n\n@app.exception_handler(RequestValidationError)\nasync def validation_exception_handler(request: Request, exc: RequestValidationError):\n    first_error = exc.errors()[0] if exc.errors() else {}\n    field = ".".join(str(loc) for loc in first_error.get("loc", []))\n    message = f"{field}: {first_error.get(\'msg\', \'invalid input\')}" if field else "Invalid request data"\n    return JSONResponse(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, content=envelope("VALIDATION_ERROR", message))\n\n@app.exception_handler(Exception)\nasync def unhandled_exception_handler(request: Request, exc: Exception):\n    logger.exception("unhandled exception on %s", request.url.path)\n    return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content=envelope("INTERNAL_ERROR", "An unexpected error occurred"))\n\nclass ItemIn(BaseModel):\n    name: str\n    price: float\n\n@app.post("/items")\nasync def create_item(payload: ItemIn):\n    if payload.price < 0:\n        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="price must be non-negative")\n    return {"name": payload.name, "price": payload.price}\n```\n\nAPI examples across all three failure paths:\n\n```text\nPOST /items  Body: {"name": "Book", "price": -5}\n-> 400 {"error": {"code": "HTTP_400", "message": "price must be non-negative"}}\n\nPOST /items  Body: {"name": "Book"}                     (missing required "price")\n-> 422 {"error": {"code": "VALIDATION_ERROR", "message": "body.price: Field required"}}\n\nPOST /items  Body: {"name": "Book", "price": "not-a-number"}\n-> 422 {"error": {"code": "VALIDATION_ERROR", "message": "body.price: Input should be a valid number"}}\n```\n\nUsing a Pydantic `ErrorResponse` model to build the envelope (rather than a raw dict literal) is deliberate: it keeps the error SHAPE itself validated and consistent, and it can be declared as `responses={400: {"model": ErrorResponse}, 422: {"model": ErrorResponse}}` on path operations so the error contract shows up correctly in the generated OpenAPI docs too, not just the success responses.',
    productionExample:
