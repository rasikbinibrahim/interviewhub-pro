...
const QUESTION_SEEDS: QuestionSeed[] = [
  {
    ...
    deepExplanation:
      '```text\nFastAPI router     — HTTP concerns only: parse request, call service, shape response\n  |\nService           — business logic, orchestrates one or more repositories, enforces rules\n  |\nRepository        — data-access abstraction: create/get/update/delete/list for ONE entity\n  |\nSQLAlchemy (ORM)  — maps Python objects <-> rows, builds SQL via select()/insert()/etc, manages Session\n  |\nPostgreSQL        — actual storage/execution engine\n```\n\n```python\nfrom sqlalchemy import select\nfrom sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column\n\nclass Base(DeclarativeBase):\n    pass\n\nclass User(Base):\n    __tablename__ = "users"\n    id: Mapped[int] = mapped_column(primary_key=True)\n    email: Mapped[str] = mapped_column(unique=True)\n\n# ORM query — returns actual User objects, tracked by the session\'s identity map\nstmt = select(User).where(User.email == "ada@example.com")\n\n# Core-only query — returns plain rows, no object mapping/tracking at all\nfrom sqlalchemy import table, column\nusers_table = table("users", column("id"), column("email"))\ncore_stmt = select(users_table).where(users_table.c.email == "ada@example.com")\n```\n\nBoth `stmt` and `core_stmt` compile to the same underlying SQL (`SELECT users.id, users.email FROM users WHERE users.email = :email_1`) — the ORM layer adds object identity, relationship traversal, and change tracking on top, at the cost of some additional bookkeeping overhead the raw Core layer does not pay. Most production FastAPI + PostgreSQL applications use the ORM for typical CRUD/entity work (where object identity and relationships genuinely help) and drop down to Core (or raw `text()` SQL) for reporting/analytics queries that do not map cleanly to a single entity (e.g. a multi-table aggregation query that returns ad hoc computed columns, not a `User` or `Order`).',
    productionExample:
      'A production repository layer typically uses the ORM\'s `select(Model)` for standard entity CRUD (benefiting from relationship loading and identity-map deduplication), but a separate `analytics` or `reporting` module often drops to Core-level `select()` over ad hoc column expressions (joins, aggregates, window functions) that do not correspond to any single mapped entity — using the ORM there would provide no benefit and would obscure the query\'s actual shape.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nasync with async_session_factory() as session:\n    user = User(email="ada@example.com", full_name="Ada Lovelace")\n    session.add(user)            # registers the object as PENDING — no SQL sent yet\n    session.add_all([user2, user3])   # register multiple objects at once\n\n    await session.flush()         # sends the pending INSERT(s) now, still inside the open transaction;\n                                    # user.id is now populated (from the DB-generated identity/sequence)\n\n    await session.refresh(user)   # re-SELECTs the row from the DB, refreshing all attributes\n                                    # (e.g. to pick up a server_default value computed by the database)\n\n    await session.commit()        # flushes any remaining pending changes AND commits the transaction —\n                                    # changes are now permanent and visible to OTHER sessions/connections\n\n    # if anything raised an exception before commit:\n    # await session.rollback()    # discards ALL pending changes in this session, reverting to last commit\n```\n\n`flush()` vs `commit()`, precisely: `flush()` sends pending SQL (INSERT/UPDATE/DELETE) to the database so subsequent queries WITHIN THE SAME transaction can see the effects (e.g. a foreign key needing a just-inserted parent\'s id) — but the transaction is still open, and a crash or rollback after a flush-but-before-commit still discards everything. `commit()` performs an implicit flush AND then issues the actual `COMMIT`, ending the transaction and making the change durable/visible to others. AUTOFLUSH (on by default) means SQLAlchemy automatically flushes pending changes before executing any QUERY within the session, so reads see your own session\'s uncommitted writes — this is why you rarely need to call `flush()` manually except when you specifically need a generated PK value before continuing.\n\nThe IDENTITY MAP concretely: `user_a = await session.get(User, 1)` and `user_b = await session.get(User, 1)` (same session, same PK) return the exact SAME Python object (`user_a is user_b` is `True`) — SQLAlchemy recognizes the row is already loaded and returns the cached instance rather than constructing a duplicate. This is what makes in-memory mutation-then-flush work correctly (you are always mutating the ONE tracked instance, never a stale duplicate) and is scoped per-Session — a DIFFERENT session querying the same row gets its OWN separate object.',
    productionExample:
      'A checkout flow that needs the newly-created order\'s auto-generated `id` to immediately create related `OrderItem` rows in the same request calls `session.flush()` right after `session.add(order)` — this sends the INSERT and populates `order.id` without yet committing the whole transaction, so if a LATER step in the same request fails (e.g. inventory check fails), `session.rollback()` still correctly undoes the order insert along with everything else, keeping the entire checkout atomic.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\n# THE BUG — N+1 queries\nusers = (await session.execute(select(User).limit(100))).scalars().all()   # query #1: 100 users\nfor user in users:\n    print(user.orders)   # LAZY LOAD: each access triggers its OWN separate query — 100 MORE queries!\n# total: 1 + 100 = 101 queries for what should be a simple report\n\n# FIX #1 — selectinload: 2 queries total, regardless of how many users\nfrom sqlalchemy.orm import selectinload\n\nstmt = select(User).options(selectinload(User.orders)).limit(100)\nusers = (await session.execute(stmt)).scalars().all()\n# query #1: SELECT * FROM users LIMIT 100\n# query #2: SELECT * FROM orders WHERE customer_id IN (1, 2, 3, ..., 100)  -- ONE query for ALL users\' orders\nfor user in users:\n    print(user.orders)   # no additional query — already loaded and attached\n\n# FIX #2 — joinedload: 1 query total, via LEFT JOIN\nfrom sqlalchemy.orm import joinedload\n\nstmt = select(User).options(joinedload(User.orders)).limit(100)\nusers = (await session.execute(stmt)).unique().scalars().all()   # .unique() REQUIRED — see below\n# ONE query: SELECT users.*, orders.* FROM users LEFT JOIN orders ON orders.customer_id = users.id LIMIT 100\n# (LIMIT here is subtly risky — see the LIMIT caveat below)\n```\n\n`selectinload` vs `joinedload`, when to prefer which: `selectinload` issues a SEPARATE query (2 total) using `IN (...)`, which means it composes SAFELY with `LIMIT`/`OFFSET` on the parent query (the parent query\'s row count is unaffected by how many children each has) — the correct default for ONE-TO-MANY relationships, especially when a parent might have MANY children (avoiding a huge duplicated result set). `joinedload` issues ONE query via LEFT JOIN, avoiding the second round trip entirely, but the result set contains one row PER CHILD (a user with 5 orders appears 5 times, once per order, with the user\'s columns repeated) — this is fine and efficient for ONE-TO-ONE or "small, bounded" one-to-many relationships, but DANGEROUS combined with `LIMIT` on the parent (the LIMIT applies to the JOINED row count, not the parent count, potentially truncating parents unexpectedly) and requires `.unique()` on the result to deduplicate the repeated parent objects (which the identity map handles correctly, but the raw row count from the query itself is still inflated).\n\nThe `.unique()` requirement specifically: without it, iterating a `joinedload` result with a one-to-many relationship yields the SAME parent User object multiple times (once per matched order row) in the returned list — `.unique()` deduplicates based on identity-map object identity, giving you back the expected one-entry-per-user list.',
    productionExample:
      'An admin dashboard listing "100 recent orders with their customer and line items" originally took 3-4 seconds due to N+1 queries (1 for orders, 100 for each order\'s customer, 100 more for each order\'s items — over 200 queries); adding `.options(selectinload(Order.customer), selectinload(Order.items))` collapsed it to exactly 3 queries total and brought response time under 50ms — this exact fix (adding `selectinload` options) is one of the single highest-leverage FastAPI + SQLAlchemy performance changes available, often with zero other code changes required.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nclass InsufficientInventoryError(Exception):\n    pass\n\nclass CheckoutService:\n    def __init__(self, session: AsyncSession):\n        self._session = session\n\n    async def checkout(self, customer_id: int, items: list[tuple[int, int]]) -> Order:\n        # items: list of (product_id, quantity)\n        order = Order(customer_id=customer_id, total=0, status="pending")\n        self._session.add(order)\n        await self._session.flush()   # populate order.id for the order_items\' foreign key\n\n        total = 0.0\n        for product_id, quantity in items:\n            # SELECT FOR UPDATE — pessimistic lock, prevents a concurrent checkout from\n            # overselling the same inventory row (see Module 18\'s locking question for depth)\n            stmt = select(Inventory).where(Inventory.product_id == product_id).with_for_update()\n            inventory = (await self._session.execute(stmt)).scalar_one()\n\n            if inventory.quantity < quantity:\n                raise InsufficientInventoryError(f"Not enough stock for product {product_id}")\n\n            inventory.quantity -= quantity   # tracked ORM mutation — becomes an UPDATE on flush/commit\n\n            product = await self._session.get(Product, product_id)\n            unit_price = product.price\n            self._session.add(OrderItem(order_id=order.id, product_id=product_id, quantity=quantity, unit_price=unit_price))\n            total += float(unit_price) * quantity\n\n        order.total = total\n        self._session.add(Payment(order_id=order.id, amount=total, status="pending"))\n\n        # NO commit here — the caller (request-scoped session dependency) commits once, at the\n        # end of the request, making order + all order_items + inventory updates + payment atomic\n        return order\n```\n\nIf `InsufficientInventoryError` is raised partway through the loop (say, on the second of three items), everything that already happened in THIS transaction — the `order` insert, the FIRST item\'s inventory decrement and `order_item` insert — is still only PENDING in the session\'s current transaction, not committed. The exception propagates up through the service, through the router\'s exception handling, and reaches the `get_db()` dependency\'s `except` block, which calls `session.rollback()` — undoing ALL of it atomically, exactly as if none of the checkout had ever been attempted. No manual "undo the order I just created" cleanup code is needed anywhere, because nothing was ever actually committed.\n\nThe `with_for_update()` pessimistic lock on the inventory row is essential for correctness under CONCURRENT checkouts of the same product: without it, two simultaneous checkouts could both read `quantity=1`, both decide there is enough stock, and both proceed — overselling by one unit. Locking the row for the duration of this transaction forces the second concurrent checkout to WAIT until the first transaction commits or rolls back, at which point it re-reads the now-updated (or reverted) quantity.',
    productionExample:
  ...
  },
  {
    ...
    deepExplanation:
      '```bash\nalembic init alembic                          # one-time: sets up the migrations/ directory + alembic.ini\nalembic revision --autogenerate -m "add phone to users"   # drafts a new revision by diffing models vs DB\nalembic upgrade head                           # applies all pending revisions, up to the latest\nalembic downgrade -1                            # reverts exactly one revision\nalembic history                                  # shows the full revision chain\n```\n\n```python\n\"\"\"add phone to users\n\nRevision ID: a1b2c3d4e5f6\nRevises: 9f8e7d6c5b4a\n\"\"\"\nfrom alembic import op\nimport sqlalchemy as sa\n\nrevision = "a1b2c3d4e5f6"\ndown_revision = "9f8e7d6c5b4a"\n\ndef upgrade() -> None:\n    op.add_column("users", sa.Column("phone", sa.String(20), nullable=True))\n\ndef downgrade() -> None:\n    op.drop_column("users", "phone")\n```\n\nSafely adding a `NOT NULL` column to a large, already-populated production table (a classic "how would you do this without downtime" senior question) requires MULTIPLE separate migrations/deploys, not one:\n\n```python\n# migration 1: add the column as NULLABLE first — instant, no table rewrite/lock in modern PostgreSQL\ndef upgrade():\n    op.add_column("users", sa.Column("phone", sa.String(20), nullable=True))\n\n# (deploy application code that starts WRITING phone on every new/updated row, backfill existing rows\n#  in BATCHES via a separate script — not a single giant UPDATE that locks the whole table)\n\n# migration 2 (LATER, once 100% of rows are confirmed backfilled): enforce NOT NULL\ndef upgrade():\n    op.alter_column("users", "phone", nullable=False)\n```\n\nMigration CONFLICTS in a team setting happen when two developers each branch off the same `down_revision` and create migrations independently — Alembic detects this as multiple "heads" (`alembic heads` shows more than one), and the fix is either manually editing one migration\'s `down_revision` to chain properly after the other, or creating an explicit MERGE revision (`alembic merge heads`) that has both as parents — this is a routine, expected part of team-based Alembic workflows, not a sign something went wrong.\n\nAutogenerate limitations worth knowing explicitly: it correctly detects added/removed tables and columns and most type/constraint changes, but does NOT reliably detect a column RENAME (it sees this as a drop + add, LOSING existing data unless you manually edit the generated migration to use `op.alter_column(..., new_column_name=...)` instead), and NEVER generates DATA migrations (backfilling values) — those must always be hand-written.',
    productionExample:
      'A production team renaming `users.name` to `users.full_name` ran `alembic revision --autogenerate`, got a migration that DROPPED the `name` column and ADDED a new empty `full_name` column (autogenerate\'s default drop+add interpretation of what looked like a rename), and caught it in code review before it shipped — the corrected migration used `op.alter_column("users", "name", new_column_name="full_name")` instead, preserving all existing data, illustrating why autogenerated migrations must always be reviewed, never applied blindly.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\nfrom datetime import datetime\nfrom sqlalchemy import func\nfrom sqlalchemy.orm import Mapped, mapped_column, declared_attr\n\nclass AuditMixin:\n    created_at: Mapped[datetime] = mapped_column(server_default=func.now())\n    updated_at: Mapped[datetime] = mapped_column(server_default=func.now(), onupdate=func.now())\n    # onupdate=func.now() -> SQLAlchemy automatically sets this on every UPDATE issued through the ORM\n    # (note: this is an ORM-level hook, not a DB-level trigger — a raw SQL UPDATE bypassing the ORM\n    # would NOT update it; a DB trigger is needed for that guarantee, see Module 16\'s trigger question)\n\nclass SoftDeleteMixin:\n    deleted_at: Mapped[datetime | None] = mapped_column(default=None)\n\n    @property\n    def is_deleted(self) -> bool:\n        return self.deleted_at is not None\n\nclass Product(Base, AuditMixin, SoftDeleteMixin):\n    __tablename__ = "products"\n    id: Mapped[int] = mapped_column(primary_key=True)\n    name: Mapped[str]\n    price: Mapped[float]\n\nclass SoftDeleteRepository:\n    """Base repository that transparently excludes soft-deleted rows by default."""\n    model: type[SoftDeleteMixin]\n\n    def __init__(self, session: AsyncSession):\n        self._session = session\n\n    def _base_query(self, *, include_deleted: bool = False):\n        stmt = select(self.model)\n        if not include_deleted:\n            stmt = stmt.where(self.model.deleted_at.is_(None))\n        return stmt\n\n    async def soft_delete(self, obj_id: int) -> bool:\n        obj = await self._session.get(self.model, obj_id)\n        if obj is None or obj.is_deleted:\n            return False\n        obj.deleted_at = func.now()   # ORM-level mutation; becomes an UPDATE ... SET deleted_at = NOW()\n        return True\n\n    async def restore(self, obj_id: int) -> bool:\n        obj = await self._session.get(self.model, obj_id)\n        if obj is None or not obj.is_deleted:\n            return False\n        obj.deleted_at = None\n        return True\n\nclass ProductRepository(SoftDeleteRepository):\n    model = Product\n\n    async def list(self, *, include_deleted: bool = False) -> list[Product]:\n        stmt = self._base_query(include_deleted=include_deleted).order_by(Product.id)\n        return list((await self._session.execute(stmt)).scalars().all())\n```\n\nA subtlety worth stating precisely: a soft-deleted row STILL EXISTS in the table and still occupies index space and still participates in FOREIGN KEY relationships — a `deleted_at IS NOT NULL` filter needs to be applied CONSCIOUSLY everywhere the entity is queried (which is exactly why centralizing it in a `SoftDeleteRepository._base_query()` helper, rather than repeating `.where(deleted_at.is_(None))` in every method, matters) — a `SELECT` query against this table with a raw SQL client or a different service that forgets this filter will silently see "deleted" rows. A PARTIAL INDEX (`CREATE INDEX ... WHERE deleted_at IS NULL`) is the standard companion technique for keeping lookups on the (much more common) non-deleted subset fast, covered in Module 18.',
    productionExample:
