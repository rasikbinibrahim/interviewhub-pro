...
const QUESTION_SEEDS: QuestionSeed[] = [
  ...
  },
  {
    ...
    deepExplanation:
      "```sql\nCREATE INDEX idx_users_email_status ON users (email, status);\n\n-- USES the index efficiently (leftmost column, or leftmost+next both present)\nSELECT * FROM users WHERE email = \\\n\nStep 1 — Understand the topic.\nTopic: Composite indexes, the leftmost-prefix rule, partial indexes, and expression indexes\n\nStep 2 — Easy method.\nIdentify the boundary/rule first, then trace a small valid example and one failure case.\n\nStep 3 — WITHOUT BUILT-IN / CORE implementation:\n```python\nEXPLAIN ANALYZE\nSELECT *\nFROM orders\nWHERE status = 'refunded';\n```\n\nStep 4 — WITH BUILT-IN / PYTHONIC / FRAMEWORK / STANDARD-LIBRARY implementation:\n```python\nCREATE INDEX idx_orders_status\nON orders(status);\n```\n\nStep 5 — Example result:\n```text\nplanner chooses an index when selective\n```\n\nStep 6 — Complexity / trade-off:\nIndexes speed selective reads but cost write work and storage.\n\nStep 7 — Edge cases:\nCheck empty/null input, invalid values, duplicate data, concurrency, transaction/lifecycle boundaries, large datasets, security exposure, and migration/rollback concerns when relevant.\n\nStep 8 — Interview takeaway:\nExplain the source concept first, then the manual implementation, the framework/standard alternative, complexity, failure mode, and the production reason for choosing one approach.",
    productionExample:
      "A `users` table query pattern of \"look up by email, and sometimes filter to active-only\" was served by ONE composite index `(email, status)` rather than two separate single-column indexes — this both satisfies the common `WHERE email = ?` case and the `WHERE email = ? AND status = ?` case from a single, smaller index, while a hypothetical index on `status` alone (a low-cardinality column, mostly \\\n\nCoding practice: explain the manual/core approach first, then the framework/Pythonic/standard-library alternative for **Composite indexes, the leftmost-prefix rule, partial indexes, and expression indexes**.",
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      "```sql\nEXPLAIN ANALYZE\nSELECT o.id, o.total, c.name\nFROM orders o\nJOIN customers c ON c.id = o.customer_id\nWHERE o.status = \\\n\nStep 1 — Understand the topic.\nTopic: Reading EXPLAIN ANALYZE — seq scan vs index scan, nested loop vs hash join, cost vs actual time\n\nStep 2 — Easy method.\nIdentify the boundary/rule first, then trace a small valid example and one failure case.\n\nStep 3 — WITHOUT BUILT-IN / CORE implementation:\n```python\nEXPLAIN ANALYZE\nSELECT *\nFROM orders\nWHERE status = 'refunded';\n```\n\nStep 4 — WITH BUILT-IN / PYTHONIC / FRAMEWORK / STANDARD-LIBRARY implementation:\n```python\nCREATE INDEX idx_orders_status\nON orders(status);\n```\n\nStep 5 — Example result:\n```text\nplanner chooses an index when selective\n```\n\nStep 6 — Complexity / trade-off:\nIndexes speed selective reads but cost write work and storage.\n\nStep 7 — Edge cases:\nCheck empty/null input, invalid values, duplicate data, concurrency, transaction/lifecycle boundaries, large datasets, security exposure, and migration/rollback concerns when relevant.\n\nStep 8 — Interview takeaway:\nExplain the source concept first, then the manual implementation, the framework/standard alternative, complexity, failure mode, and the production reason for choosing one approach.",
    productionExample:
      "An on-call engineer diagnosing a suddenly-slow production endpoint ran `EXPLAIN ANALYZE` on the underlying query and found `actual rows` was 50x higher than the planner\\\n\nCoding practice: explain the manual/core approach first, then the framework/Pythonic/standard-library alternative for **Reading EXPLAIN ANALYZE — seq scan vs index scan, nested loop vs hash join, cost vs actual time**.",
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      "```sql\n-- multi-tenant table, one physical table shared across all tenants\nCREATE TABLE documents (\n    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,\n    tenant_id BIGINT NOT NULL,\n    title TEXT NOT NULL,\n    content TEXT\n);\n\nALTER TABLE documents ENABLE ROW LEVEL SECURITY;\n\n-- a policy: a role can only see/modify rows where tenant_id matches a session-level setting\nCREATE POLICY tenant_isolation ON documents\n    USING (tenant_id = current_setting(\\\n\nStep 1 — Understand the topic.\nTopic: Database security: least-privilege roles, row-level security, and encryption in transit/at rest\n\nStep 2 — Easy method.\nIdentify the boundary/rule first, then trace a small valid example and one failure case.\n\nStep 3 — WITHOUT BUILT-IN / CORE implementation:\n```python\nEXPLAIN ANALYZE\nSELECT *\nFROM orders\nWHERE customer_id = 42;\n```\n\nStep 4 — WITH BUILT-IN / PYTHONIC / FRAMEWORK / STANDARD-LIBRARY implementation:\n```python\nCREATE INDEX idx_orders_customer\nON orders(customer_id);\n```\n\nStep 5 — Example result:\n```text\nquery becomes measurable and optimizable\n```\n\nStep 6 — Complexity / trade-off:\nUse evidence from EXPLAIN ANALYZE, not guesswork.\n\nStep 7 — Edge cases:\nCheck empty/null input, invalid values, duplicate data, concurrency, transaction/lifecycle boundaries, large datasets, security exposure, and migration/rollback concerns when relevant.\n\nStep 8 — Interview takeaway:\nExplain the source concept first, then the manual implementation, the framework/standard alternative, complexity, failure mode, and the production reason for choosing one approach.",
    productionExample:
      "A SaaS platform storing all tenants\\\n\nCoding practice: explain the manual/core approach first, then the framework/Pythonic/standard-library alternative for **Database security: least-privilege roles, row-level security, and encryption in transit/at rest**.",
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      "```python\n# VULNERABLE — string-formats untrusted input directly into the SQL text\ndef get_user_by_email_UNSAFE(conn, email: str):\n    query = f\"SELECT * FROM users WHERE email = \\\n\nStep 1 — Understand the topic.\nTopic: SQL injection — the vulnerable f-string pattern, a real payload, and the parameterized fix\n\nStep 2 — Easy method.\nIdentify the boundary/rule first, then trace a small valid example and one failure case.\n\nStep 3 — WITHOUT BUILT-IN / CORE implementation:\n```python\nquery = \"\"\"\nSELECT *\nFROM users\nWHERE email = %s\n\"\"\"\ncursor.execute(\n    query,\n    (email,),\n)\n```\n\nStep 4 — WITH BUILT-IN / PYTHONIC / FRAMEWORK / STANDARD-LIBRARY implementation:\n```python\ncursor.execute(\n    \"SELECT * FROM users WHERE email = %s\",\n    (email,),\n)\n```\n\nStep 5 — Example result:\n```text\nparameterized query\n```\n\nStep 6 — Complexity / trade-off:\nParameter binding separates data from SQL syntax and is the primary SQL-injection defense.\n\nStep 7 — Edge cases:\nCheck empty/null input, invalid values, duplicate data, concurrency, transaction/lifecycle boundaries, large datasets, security exposure, and migration/rollback concerns when relevant.\n\nStep 8 — Interview takeaway:\nExplain the source concept first, then the manual implementation, the framework/standard alternative, complexity, failure mode, and the production reason for choosing one approach.",
    productionExample:
      "A search endpoint built with `f\"SELECT * FROM products WHERE name ILIKE \\\n\nCoding practice: explain the manual/core approach first, then the framework/Pythonic/standard-library alternative for **SQL injection — the vulnerable f-string pattern, a real payload, and the parameterized fix**.",
    bestPractices: [
