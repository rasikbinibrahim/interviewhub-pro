...
const QUESTION_SEEDS: QuestionSeed[] = [
  ...
  },
  {
    ...
    productionExample:
      'Web frameworks (`@app.route("/users")`, `@login_required`, `@cache.cached(timeout=60)`) are all decorators in exactly this shape — route registration decorators return the original function unchanged (just registering it as a side effect), while `login_required`/caching decorators wrap the function\'s actual execution with additional behavior, exactly like `require_role` above.',
    bestPractices: [
  ...
  },
  {
    ...
    deepExplanation:
      '```python\n# Q1\nx = 10\nsquares = [x for x in range(3)]\nprint(x)\n# Answer: 2 — in Python 3, list comprehensions have their OWN scope, so the\n# comprehension\'s `x` does NOT leak out... EXCEPT here it does look like it changed `x`?\n# Actually the correct answer is: x is still 10. The comprehension\'s `x` is local\n# to the comprehension in Python 3 (unlike Python 2). Many candidates incorrectly\n# guess 2 — this specifically tests knowledge of the Python 2 -> 3 scoping change.\n\n# Q2\ndef gen():\n    print("start")\n    yield 1\n    print("middle")\n    yield 2\n    print("end")\n\ng = gen()\nprint("created")\nnext(g)\n# Answer:\n# created\n# start\n# 1  <- (this is the return value of next(g), if printed explicitly; next(g) alone just returns 1)\n# Nothing from gen() prints until the FIRST next() call — generator bodies do not run at creation time.\n\n# Q3\ndef make_multipliers():\n    return [lambda x: x * i for i in range(3)]\n\nfns = make_multipliers()\nprint([f(10) for f in fns])\n# Answer: [20, 20, 20] — classic late-binding closure trap (see the closures question above);\n# all three lambdas share the same `i`, which is 2 by the time they are called.\n\n# Q4\ndef accumulate(value, bucket=[]):\n    bucket.append(value)\n    return bucket\n\nprint(accumulate(1))\nprint(accumulate(2))\n# Answer: [1]  then  [1, 2] — the mutable default argument trap (Module 2), the SAME list\n# object is reused across calls that omit the `bucket` argument.\n```\nThese four together cover the four most commonly-tested "gotcha" categories in senior Python interviews: comprehension scoping (Python 3 change from Python 2), generator laziness (nothing runs until pulled), closure late-binding, and mutable-default-argument aliasing — a candidate who can correctly explain all four, not just recite the "gotcha," demonstrates real fluency.',
    productionExample:
